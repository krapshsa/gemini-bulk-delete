# ChatGPT Bulk Delete

A Tampermonkey script for ChatGPT that enables bulk deletion of conversations.

## Features

- **Batch Selection**: Checkboxes added to the conversation list for multiple selection.
- **Select All**: Quickly select all visible conversations.
- **Bulk Delete**: A dedicated delete button to remove all selected items at once.
- **Fast Sequential Submission**: Submits each deletion through ChatGPT's UI without waiting for the sidebar row to disappear.
